...
# --- FILTRE EMA + REJET D'OUTLIERS ---
EMA_ALPHA    = 0.2   # Lissage : 0.1 = très lisse, 0.5 = très réactif
OUTLIER_MAX  = 0.5   # Saut max accepté en mètres entre 2 mesures
...
def run_multimonitor():
    ...
    positions = {s: {"X": None, "Y": None, "X_ema": None, "Y_ema": None, "last_update": 0} for s in SENSORS}
            ...
            while ser.in_waiting > 0:
                ...
                if sweep == 0:
                    ...
                    if all(v is not None for v in last_angles[s_id].values()):
                        ...
                        if X is not None:
                            pos = positions[s_id]
                            # Rejet d'outlier : on ignore si le saut est trop grand
                            if pos["X_ema"] is not None:
                                dist = math.sqrt((X - pos["X_ema"])**2 + (Y - pos["Y_ema"])**2)
                                if dist > OUTLIER_MAX:
                                    # Outlier détecté → on ignore cette mesure
                                    last_angles[s_id] = {b: None for b in BASESTATIONS}
                                    continue
                            # EMA
                            if pos["X_ema"] is None:
                                pos["X_ema"] = X
                                pos["Y_ema"] = Y
                            else:
                                pos["X_ema"] = EMA_ALPHA * X + (1 - EMA_ALPHA) * pos["X_ema"]
                                pos["Y_ema"] = EMA_ALPHA * Y + (1 - EMA_ALPHA) * pos["Y_ema"]
                            pos["X"] = pos["X_ema"]
                            pos["Y"] = pos["Y_ema"]
