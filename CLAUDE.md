...
TS4231 sensors → PIO+DMA capture (core 1)
              ...
              → triangulation (solve3d / DLT)
              → MAVLink ODOMETRY @ 10 Hz → Pixhawk TELEM2
```

### BS pose delivery pipeline

```
FC flash (LH2_BS* params) → lh2_bs_params.lua (param:get()) → NAMED_VALUE_FLOAT @ 1 Hz
                                                              → Pico (boot-wait + 10 Hz refresh)
```

`lh2_bs_params.lua` runs on the FC and pushes all 25 BS pose values every second via MAVLink NAMED_VALUE_FLOAT. The Pico waits for all 25 at boot, then refreshes `BS_POSES` every 100 ms — so GCS UI pose changes take effect within ~1–2 s without rebooting the Pico.
...
- `bs_poses_cal.h` has been removed — BS geometry is delivered at runtime via MAVLink NAMED_VALUE_FLOAT from `utils/lua_scripts/lh2_bs_params.lua` on the FC SD card.
- Room geometry lives in one place: `utils/lua_scripts/lh2_bs_params.lua`. Edit FC params via the GCS UI or Mission Planner — the Pico picks up changes within ~1–2 s; no firmware recompile or reboot needed.
