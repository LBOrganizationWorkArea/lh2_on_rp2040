...
def capture_pose(
    ...
    if lh2p_frames:
        clean_frames = select_clean_lh2p_frames(
            lh2p_frames,
            max_sensor_spread_ticks=max_sensor_spread_ticks,
            max_sensor_spread_deg=max_sensor_spread_deg,
        ...
        for frame in clean_frames:
            for data in frame_to_observations(frame, factory_calibs, angle_source=angle_source):
...
def main():
    ...
    parser.add_argument(
        "--max-sensor-spread-deg",
        type=float,
        default=0.0,
        help="Optional internal LH2P/LH2P2 frame spread filter in degrees. If set, this is easier to tune than ticks.",
    )
    parser.add_argument(
        "--factory-calibs",
        default="auto",
    ...
    parser.add_argument(
        "--angle-source",
        choices=["offsets", "lfsr", "auto"],
        default="offsets",
        help="Use LH2P offsets_24, LH2P2 lfsr_locations, or auto-prefers-lfsr when converting to angles.",
    )
