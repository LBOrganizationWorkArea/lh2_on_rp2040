...
## 4. Fit initial anchored Lighthouse geometry
...
The wave should pass through places where the drone will actually fly. You can rotate during this recording; the next step estimates a 6D pose for each wave frame.

## 6. Refine with points + wave

This is the main v10 fit: known points anchor the room, and the moving wave adds dense rigid-body constraints.
...
```text
config/lighthouse_geometry_points_plus_wave.json
```

If you are testing with the drone held vertical/head down, pass the temporary vertical layout:

```powershell
py .\tools\10_refine_geometry_points_plus_wave.py --layout config\sensors_layout_vertical_head_down.json
```
