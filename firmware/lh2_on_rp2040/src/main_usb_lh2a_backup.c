...
// Envoie les donnees serie toutes les 100 ms
...
int main() {
    ...
    printf("LH2A,sensor,sweep,basestation,polynomial,lfsr_location,raw_angle_urad\n");
...
void print_sensor_csv(uint8_t sensor_id, db_lh2_t *lh2) {
    for (uint8_t sweep = 0; sweep < LH2_SWEEP_COUNT; sweep++) {
        for (uint8_t basestation = 0; basestation < LH2_BASESTATION_COUNT; basestation++) {
            ...
            float angle_rad = db_lh2_location_to_angle_rad(lfsr_location, sweep);
            int32_t angle_urad = (int32_t)(angle_rad * 1000000.0f);

            printf("LH2A,%u,%u,%u,%u,%lu,%ld\n",
                   sensor_id,
                   sweep,
                   basestation,
                   polynomial,
