...
#define LH2_LFSR_CYCLE_TICKS           120000.0f                  ///< LH2 LFSR cycle length used to map decoded location to sweep angle
...
/// Last decoded LH2 pulse/frame for one sensor.
typedef struct __attribute__((packed)) {
    absolute_time_t timestamp;            ///< timestamp of the capture, currently taken when the fixed PIO sample window completes
    uint64_t        bits_sweep;           ///< demodulated bits from this capture
    uint32_t        lfsr_bits;            ///< 17-bit LFSR state used for the location search
    uint32_t        lfsr_location;        ///< decoded LFSR offset in the selected polynomial sequence
    uint8_t         sensor;               ///< TS4231 sensor index
    uint8_t         sweep;                ///< firmware sweep slot, 0 or 1
    uint8_t         basestation;          ///< base station channel, polynomial >> 1
    uint8_t         selected_polynomial;  ///< selected LH2 polynomial, 0..31
    int8_t          bit_offset;           ///< bit offset found by the demodulator
    bool            valid;                ///< true once at least one frame has been decoded
} db_lh2_decoded_frame_t;
...
typedef struct {
    ...
    db_lh2_decoded_frame_t    latest_frame;                                        ///< most recent decoded frame for event-driven consumers
    volatile uint32_t         latest_frame_seq;                                    ///< increments whenever latest_frame is updated
...
/**
 * @brief Convert a decoded LH2 LFSR location to a raw sweep angle.
 *
 * This follows the same shape as Bitcraze LH2 pulse_processor_v2.c:
 * angle = offset * 2pi / period - pi +/- pi/3
 *
 * The RP2040 decoder exposes an LFSR sequence location instead of Bitcraze's
 * FPGA timestamp offset, so LH2_LFSR_CYCLE_TICKS is used as the period.
 *
 * @param[in] lfsr_location decoded LFSR sequence location
 * @param[in] sweep         sweep slot, 0 or 1
 *
 * @return raw sweep angle in radians
 */
float db_lh2_location_to_angle_rad(uint32_t lfsr_location, uint8_t sweep);
