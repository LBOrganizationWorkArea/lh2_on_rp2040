...
void db_lh2_init(db_lh2_t *lh2, uint8_t sensor, const uint8_t gpio_d, const uint8_t gpio_e) {
    ...
    memset(&lh2->latest_frame, 0, sizeof(lh2->latest_frame));
    lh2->latest_frame.valid = false;
    lh2->latest_frame_seq   = 0;
...
float db_lh2_location_to_angle_rad(uint32_t lfsr_location, uint8_t sweep) {
    ...
    const float tilt_phase = pi / 3.0f;
    const float base = ((float)lfsr_location * two_pi / LH2_LFSR_CYCLE_TICKS) - pi;

    if (sweep == 0) {
        return base + tilt_phase;
    }
...
// Check if Qualysis Mocap data is interfering with the SPI capture
    ...
    lh2->latest_frame.timestamp           = temp_timestamp;
    lh2->latest_frame.bits_sweep          = temp_bits_sweep;
    lh2->latest_frame.lfsr_bits           = temp_lfsr_bits;
    lh2->latest_frame.lfsr_location       = temp_lfsr_loc;
    lh2->latest_frame.sensor              = sensor;
    lh2->latest_frame.sweep               = sweep;
    lh2->latest_frame.basestation         = basestation;
    lh2->latest_frame.selected_polynomial = temp_selected_polynomial;
    lh2->latest_frame.bit_offset          = temp_bit_offset;
    lh2->latest_frame.valid               = true;
    lh2->latest_frame_seq++;
}
