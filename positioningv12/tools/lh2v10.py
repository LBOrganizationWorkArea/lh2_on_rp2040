...
class Lh2pFrame:
    ...
    lfsr_locations: tuple[tuple[int, int], ...] | None
    raw_line: str
...
def parse_lh2p_line(line):
    ...
    if line.startswith("LH2P2;"):
        ...
        for i in range(0, 16, 4):
            offsets.append((raw_values[i], raw_values[i + 1]))
            lfsr_locations.append((raw_values[i + 2], raw_values[i + 3]))
        ...
        return Lh2pFrame(
            ...
            lfsr_locations=tuple(lfsr_locations),
            raw_line=line,
        )
...
def offset_to_raw_angle(offset, basestation, axis, source="offsets"):
    if source == "lfsr":
        period = LFSR_PERIOD
    else:
...
def frame_to_observations(frame, factory_by_bs=None, angle_source="offsets"):
    ...
    for sensor, pair in enumerate(frame.offsets):
        value_pair = pair
        source_used = "offsets"
        if angle_source == "lfsr":
            if frame.lfsr_locations is None:
                continue
            value_pair = frame.lfsr_locations[sensor]
            source_used = "lfsr"
        elif angle_source == "auto" and frame.lfsr_locations is not None:
            value_pair = frame.lfsr_locations[sensor]
            source_used = "lfsr"

        if value_pair[0] == 0 and value_pair[1] == 0:
            continue

        raw_by_axis = {
            first_axis: offset_to_raw_angle(value_pair[0], bs, first_axis, source_used),
            second_axis: offset_to_raw_angle(value_pair[1], bs, second_axis, source_used),
        }
        ...
        value_by_axis = {first_axis: value_pair[0], second_axis: value_pair[1]}
        ...
        for axis in (0, 1):
            observations.append({
                ...
                "lfsr_location": int(value_by_axis[axis]),
                "offset_ticks": int(offset_by_axis[axis]),
                "angle_source": source_used,
...
def _frame_family_key(frame, angle_source="offsets"):
    values = _frame_axis_values(frame, angle_source=angle_source)
    if values is None:
        return None

    axis0_values, axis1_values, _period = values
    return median(axis0_values), median(axis1_values)


def _frame_axis_values(frame, angle_source="offsets"):
    axes = frame_axes(frame)
    if axes is None:
        return None

    first_axis, second_axis = axes
    use_lfsr = angle_source in ("lfsr", "auto") and frame.lfsr_locations is not None
    period = int(LFSR_PERIOD if use_lfsr else CYCLE_PERIODS[int(frame.basestation)])
    pairs = frame.lfsr_locations if use_lfsr else frame.offsets
    axis0_values = []
    axis1_values = []
    for pair in pairs:
    ...
    return axis0_values, axis1_values, period
...
def frame_internal_spread_ticks(frame, angle_source="offsets"):
    values = _frame_axis_values(frame, angle_source=angle_source)
    if values is None:
        return float("inf")

    axis0_values, axis1_values, period = values
    return max(
...
def frame_has_coherent_sensor_offsets(frame, max_sensor_spread_ticks=50000, angle_source="offsets"):
    return frame_internal_spread_ticks(frame, angle_source=angle_source) <= max_sensor_spread_ticks


def select_clean_lh2p_frames(
    frames,
    cluster_ticks=None,
    min_frames=1,
    max_sensor_spread_ticks=None,
    max_sensor_spread_deg=None,
    angle_source="offsets",
):
    by_bs = {}
    for frame in frames:
        ...
        if values is None:
            continue
        _axis0_values, _axis1_values, period = values
        frame_max_spread_ticks = max_sensor_spread_ticks
        if max_sensor_spread_deg is not None and max_sensor_spread_deg > 0:
            frame_max_spread_ticks = float(max_sensor_spread_deg) * float(period) / 360.0
        if frame_max_spread_ticks is not None and not frame_has_coherent_sensor_offsets(
            frame,
            frame_max_spread_ticks,
    ...
    for bs, bs_frames in by_bs.items():
        period = int(LFSR_PERIOD if angle_source in ("lfsr", "auto") else CYCLE_PERIODS[int(bs)])
        active_cluster_ticks = cluster_ticks
        if active_cluster_ticks is None:
            active_cluster_ticks = 6000 if angle_source in ("lfsr", "auto") else 25000
        clusters = []
        for frame in bs_frames:
            key = _frame_family_key(frame, angle_source=angle_source)
            if key is None:
                continue
            ...
            for cluster in clusters:
                ...
                if distance <= active_cluster_ticks and (best_distance is None or distance < best_distance):
            ...
            if best_cluster is None:
                ...
                keys = [_frame_family_key(item, angle_source=angle_source) for item in best_cluster["frames"]]
                best_cluster["center"] = (median(k[0] for k in keys), median(k[1] for k in keys))

        if not clusters:
            continue

        clusters.sort(key=lambda c: (len(c["frames"]), -_cluster_spread(c, period, angle_source=angle_source)), reverse=True)
...
def summarize_observation_group(values, angle_outlier_deg=8.0, min_samples=1):
    ...
    offset_values = [item["offset_ticks"] for item in kept if "offset_ticks" in item]
    ...
    if offset_values:
        summary["offset_ticks"] = float(median(offset_values))
        summary["median_offset_ticks"] = float(median(offset_values))
    if raw_angle_values:
...
def _cluster_spread(cluster, period=None, angle_source="offsets"):
    keys = [_frame_family_key(item, angle_source=angle_source) for item in cluster["frames"]]
    center0, center1 = cluster["center"]
    if not keys:
        return float("inf")
    if period is None:
