...
def main():
    ...
    parser.add_argument("--recapture", help="Comma-separated pose names to overwrite when used with --resume.")
    parser.add_argument("--no-backup", action="store_true", help="Do not create a backup copy of an existing output file.")
    ...
    recapture = {item.strip() for item in args.recapture.split(",")} if args.recapture else set()
    ...
    with serial.Serial(args.port, args.baudrate, timeout=0.5) as ser:
        for pose in poses:
            name = pose["name"]
            if args.resume and name in captured and name not in recapture:
