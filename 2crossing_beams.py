...
# =========================================
# DISPOSITION DES LIGHT HOUSES - XYZ ET YPR
# =========================================

# attention : les angles du yaw (et surement ceux des autres) sont anti trigo

BS_POSES = {
    4:  {"pos": (0.0, 0.0, -0.50), "rot": (0.0, 0.0, 0.0)},
    10: {"pos": (1.0, 0.0, -0.50), "rot": (0.0, 0.0, 0.0)}
}
...
# ============================================================
# GÉOMÉTRIE : CONSTRUCTION & ROTATION DU RAYON
# ============================================================
...
def rotate_vector_3d(vx, vy, vz, yaw_deg, pitch_deg, roll_deg):
    """ Rotation d'Euler Z-X-Y """
    r_y = math.radians(yaw_deg)
    r_p = math.radians(pitch_deg)
    r_r = math.radians(roll_deg)

    # Roll (Y)
    cx = vx * math.cos(r_r) + vz * math.sin(r_r)
    cy = vy
    cz = -vx * math.sin(r_r) + vz * math.cos(r_r)
    vx, vy, vz = cx, cy, cz
    
    # Pitch (X)
    cx = vx
    cy = vy * math.cos(r_p) - vz * math.sin(r_p)
    cz = vy * math.sin(r_p) + vz * math.cos(r_p)
    vx, vy, vz = cx, cy, cz
    
    # Yaw (Z)
    cx = vx * math.cos(r_y) - vy * math.sin(r_y)
    cy = vx * math.sin(r_y) + vy * math.cos(r_y)
    cz = vz
    return cx, cy, cz
...
def crossing_beams(az4, el4, az10, el10):
    v1_local = build_ray_spherical(az4, el4)
    v2_local = build_ray_spherical(az10, el10)

    d1 = rotate_vector_3d(*v1_local, *BS_POSES[4]["rot"])
    d2 = rotate_vector_3d(*v2_local, *BS_POSES[10]["rot"])
...
def main():
    print("\n===================================================")
    print(" LBEES - PARALLEL CROSSING BEAMS + CALIBRATION YAW")
    print("===================================================\n")
    ...
    # saut a la ligne pour lisibilite
    print("\n\n\n")
    ...
    while True:
        try:
            ...
            # 2. CALCUL ET AFFICHAGE (10 FPS)
            now = time.time()
            if now - last_ui > 0.1:
                valid = []
                sum_az4, sum_el4 = 0, 0
                sum_az10, sum_el10 = 0, 0
                active_count = 0
                ...
                # Remonte de 3 lignes pour nettoyer l'affichage
                ...
                if len(valid) > 0:
                    ...
                    avg_az4 = sum_az4 / active_count
                    avg_el4 = sum_el4 / active_count
                    avg_az10 = sum_az10 / active_count
                    avg_el10 = sum_el10 / active_count

                    sys.stdout.write(f"🔍 BS 4  (Gauche) | Azimut: {avg_az4:+06.2f}° | Élévation: {avg_el4:+06.2f}°\033[K\n")
                    sys.stdout.write(f"🔍 BS 10 (Droite) | Azimut: {avg_az10:+06.2f}° | Élévation: {avg_el10:+06.2f}°\033[K\n")
                    sys.stdout.write(f"🎯 BARYCENTRE     | X: {cx:+06.3f} m | Y: {cy:+06.3f} m | Z: {cz:+06.3f} m | ERR: {avg_err*100:05.1f} cm  [{len(valid)}/4]\033[K\n")
                else:
                    sys.stdout.write("🔍 Attente signal lasers... \033[K\n")
